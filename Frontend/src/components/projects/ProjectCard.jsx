...
function ProjectCard({ project, index, liked, onToggleLike }) {
  ...
  return (
    <article className="group relative h-full overflow-hidden rounded-[20px] border border-white/45 bg-white/55 shadow-[0_10px_28px_rgba(15,23,42,0.10)] backdrop-blur-xl">
      <div className="pointer-events-none absolute inset-0 rounded-[20px] p-[1px]" style={{ background: 'linear-gradient(120deg, rgba(56,126,255,0.9), rgba(132,94,255,0.86), rgba(41,214,255,0.9), rgba(56,126,255,0.9))', backgroundSize: '260% 260%' }}>
      ...
      <div className="relative z-[2] p-5">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {tags.slice(0, 3).map((tag) => (
            <span key={`${project.id || project.title}-${tag}`} className="rounded-full border border-[#d6e4ff] bg-white/78 px-2.5 py-1 text-[11px] font-semibold text-[#355487] shadow-[0_4px_14px_rgba(26,78,190,0.08)]">
              {tag}
            </span>
          ))}
        </div>
